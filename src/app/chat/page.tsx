...
export default function ChatPage() {
  ...
  useEffect(() => { if (conversations.length > 0 && !activeId) setActiveId(conversations[0].id); }, [conversations, activeId]);
  ...
  return (
    <div className="flex -my-[22px] -mx-[28px] h-[calc(100vh-44px)]">
      <div className="w-[280px] border-r border-oc-border shrink-0 overflow-y-auto" style={{ padding: "20px 14px" }}>
        <ConversationList conversations={conversations} activeId={activeId} onSelect={handleSelect} onNew={handleNew} />
        <div className="mt-auto pt-4 border-t border-oc-border">
          <div className="text-tiny font-semibold text-oc-text-muted uppercase tracking-[0.05em] mb-2">Telegram Bot</div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="border-b border-oc-border flex items-center gap-3 shrink-0" style={{ padding: "14px 20px" }}>
          <div className="flex-1 min-w-0">
            <div className="text-[15px] font-bold text-oc-text truncate">{activeConv?.title || "Chat & Commands"}</div>
            <div className="text-tiny text-oc-text-muted">Claude Sonnet 4.6 · Natural language commands</div>
          </div>
...
function EmptyState() {
  return (
    <div className="h-full flex flex-col items-center justify-center text-center">
      ...
      <div className="text-[17px] font-bold text-oc-text mb-1">Chat with Claude</div>
      ...
      <div className="flex flex-wrap justify-center gap-2 mt-5">
        {["Create a TikTok about AI", "How are my agents?", "What's the status?", "Help"].map((cmd) => (
          <span key={cmd} className="text-tiny font-mono text-oc-text-secondary bg-oc-bg border border-oc-border-light rounded-[8px] px-3 py-1.5">{cmd}</span>
        ))}
      </div>
    </div>
  );
