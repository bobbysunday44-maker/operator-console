...
const STATUS_STYLES: Record<string, { label: string; color: string; bg: string }> = {
  ...
  closed: { label: "Closed", color: "#9C9590", bg: "#F0EDE6" },
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          { label: "Errors", value: sessions.filter((s) => s.status === "error").length, color: "text-oc-red" },
        ].map((s) => (
      ...
      <div className="flex flex-col gap-3">
        {sessions.length === 0 ? (
          <div className="text-center py-8 text-tiny text-oc-text-muted">No browser sessions. Connect the Chrome extension to start.</div>
        ) : sessions.map((session) => {
          const statusStyle = STATUS_STYLES[session.status] || STATUS_STYLES.idle;
          return (
            <div key={session.id} className="p-[16px_18px] bg-oc-card border border-oc-border rounded-[10px]">
              <div className="flex items-start gap-3">
                <div className="w-10 h-10 rounded-oc-sm bg-oc-bg flex items-center justify-center text-[20px] shrink-0">🌐</div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <span className="text-small font-semibold text-oc-text">{session.site}</span>
                    <OcBadge label={statusStyle.label} color={statusStyle.color} bg={statusStyle.bg} />
                  </div>
                  <div className="text-tiny text-oc-text-secondary mb-1">{session.action}</div>
                  <div className="flex items-center gap-3 text-[9px]">
                    {session.tabId && <span className="font-mono text-oc-text-muted">Tab: {session.tabId}</span>}
                    <span className="text-oc-text-muted ml-auto">{timeAgo(session.createdAt)}</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div className="p-[14px_16px] bg-oc-bg border border-oc-border-light rounded-[10px]">
        <div className="text-tiny font-semibold text-oc-text-muted uppercase tracking-[0.05em] mb-1">Chrome Extension</div>
